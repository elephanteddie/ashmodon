...
namespace BridgePublic
{
    public class PublicStore
    {
        ...
        private CloudTableClient tableClient = null; //bridgebase
        private string user;
        private log blog;

        public PublicStore(CloudStorageAccount acct, string usr)
        {
            user = usr;
            storageAccount = acct;
            tableClient = storageAccount.CreateCloudTableClient();
            blog = new log(user, "LogsTableStorageUser", storageAccount);
            blog.g(user + " PublicStore constructed");
        }
        ...
        private async Task<CloudTable> gt(string name)
        {
            try
            {
                var table = tableClient.GetTableReference(name);
        ...
        public async Task<T> GetI<T>(string tableName, string partitionKey, string rowKey) where T : TableEntity, new()
        {
            try
            {
        ...
        public async Task<List<T>> GetP<T>(string tableName, string partitionKey) where T : TableEntity, new()
        {
            try
            {
                CloudTable table = await gt(tableName);
        ...
        public async Task<List<T>> GetAll<T>(string tableName) where T : TableEntity, new()
        {
            try
            {
        ...
        public async Task<bool> UpsertI<T>(string tableName, T item) where T : TableEntity, new()
        {
            try
            {
                CloudTable table = await gt(tableName);
        ...
        public async Task<bool> UpsertP<T>(string tableName, List<T> parts) where T : TableEntity, new()
        {
            try
            {
                CloudTable table = await gt(tableName);
        ...
        public async Task<bool> UpsertAll<T>(string tableName, List<T> partsIn) where T : TableEntity, new()
        {
            try
            {
                CloudTable table = await gt(tableName);
        ...
        public async Task<bool> deleteI<T>(string tableName, T item) where T : TableEntity, new()
        {
            try
            {
        ...
        public async Task<bool> deleteAll<T>(string tableName, List<T> items) where T : TableEntity, new()
        {
            try
            {
                CloudTable tbl = await gt(tableName);
